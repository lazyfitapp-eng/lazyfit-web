...
- Food Search + Date Boundary - Production Deploy + Smoke: deploy the locally validated Food Search Relevance Fix and Food/Dashboard selected-date consistency fix, then smoke `/food`, food search relevance, food logging, Dashboard selected-date totals, target integrity, and runtime/network cleanliness.
...
- Food/Dashboard selected-date boundary consistency fix: implemented and locally validated. Food and Dashboard now share the `Europe/Bucharest` selected-date contract, Food logs selected-date items with `timestampForSelectedDate`, both screens query with `getLocalDayBounds`, Dashboard accepts `?date=YYYY-MM-DD`, and BottomNav preserves valid `date` between Dashboard and Food. Local validation passed through `.codex-temp/food-date-boundary-validation/run.ps1`; no duplicate QA food rows were created by repaired validation. Production deployment is pending.
